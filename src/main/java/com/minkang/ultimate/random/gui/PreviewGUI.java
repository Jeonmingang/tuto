  ...
  public static void open(Main plugin, Player p, Roulette r){
    ...
    int slot=0; for(RouletteEntry e:r.getEntries()){
      ...
      if(m!=null){
        ...
        if(m.hasLore()) lore.addAll(0, new java.util.ArrayList<>(m.getLore())); m.setLore(new java.util.ArrayList<>(new java.util.LinkedHashSet<>(lore))); it.setItemMeta(m);
      }
      inv.setItem(slot++, it);
    }
    ...
    if(dm!=null){
      ...
      dm.setLore(new java.util.ArrayList<>(new java.util.LinkedHashSet<>(lore))); draw.setItemMeta(dm);
    }
    inv.setItem(49, draw);
    p.openInventory(inv);
